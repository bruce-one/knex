...
import _assign from 'lodash/assign'; /* eslint max-len:0 */
...
_assign(QueryCompiler_Oracle.prototype, {
  ...
  insert: function insert() {
    ...
    if (!Array.isArray(insertValues) && _isPlainObject(this.single.insert)) {
    ...
    if (Array.isArray(insertValues) && insertValues.length === 1 && _isEmpty(insertValues[0])) {
    ...
    if (_isEmpty(this.single.insert) && typeof this.single.insert !== 'function') {
      return '';
    }
    ...
    sql.sql = 'begin ' + _map(insertData.values, function (value) {
  ...
  select: function select() {
    ...
    var query = this.with();
    var statements = _map(components, function (component) {
      return _this2[component]();
    });
    query += _compact(statements).join(' ');
