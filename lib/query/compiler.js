...
_assign(QueryCompiler.prototype, {
  ...
  toSQL: function toSQL(method, tz) {
    ...
    var query = {
      method: method,
      options: _reduce(this.options, _assign, {}),
    ...
    _Object$defineProperties(query, {
      toNative: {
  ...
  onBetween: function onBetween(statement) {
    return this.formatter.wrap(statement.column) + ' ' + this._not(statement, 'between') + ' ' + _map(statement.value, _bind(this.formatter.parameter, this.formatter)).join(' and ');
  },
  ...
  _prepInsert: function _prepInsert(data) {
    ...
    while (++i < data.length) {
      ...
      if (i === 0) columns = _Object$keys(data[i]).sort();
      var row = new Array(columns.length);
      var keys = _Object$keys(data[i]);
      var j = -1;
      while (++j < keys.length) {
        var key = keys[j];
  ...
  _prepUpdate: function _prepUpdate(data) {
    data = _omitBy(data, _isUndefined);
    var vals = [];
    var columns = _Object$keys(data);
    var i = -1;
    while (++i < columns.length) {
  ...
  _groupsOrders: function _groupsOrders(type) {
    ...
    var sql = items.map(function (item) {
      var column = item.value instanceof Raw ? formatter.unwrapRaw(item.value) : formatter.columnize(item.value);
