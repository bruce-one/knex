...
_assign(Builder.prototype, {
  ...
  clone: function clone() {
    ...
    if (!_isUndefined(this._options)) {
      cloned._options = _clone(this._options);
    }
    if (!_isUndefined(this._queryContext)) {
      cloned._queryContext = _clone(this._queryContext);
    }
  ...
  join: function join(table, first) {
    ...
    if (typeof first === 'function') {
      ...
      if (arguments.length > 1) {
        join.on.apply(join, _toArray(arguments).slice(1));
      }
    }
  ...
  where: function where(column, operator, value) {
    ...
    if (_isObject(column) && !(column instanceof Raw)) return this._objectWhere(column);
  ...
  orWhere: function orWhere() {
    this._bool('or');
    var obj = arguments[0];
    if (_isObject(obj) && !_isFunction(obj) && !(obj instanceof Raw)) {
  ...
  whereIn: function whereIn(column, values) {
    if (Array.isArray(values) && _isEmpty(values)) return this.where(this._not());
    this._statements.push({
      grouping: 'where',
      type: 'whereIn',
      column: column,
  ...
  union: function union(callbacks, wrap) {
    if (arguments.length === 1 || arguments.length === 2 && _isBoolean(wrap)) {
      ...
      callbacks = _toArray(arguments).slice(0, arguments.length - 1);
  ...
  first: function first() {
    ...
    if (!_includes(['pluck', 'first', 'select'], _method)) {
  ...
  insert: function insert(values, returning) {
    this._method = 'insert';
    if (!_isEmpty(returning)) this.returning(returning);
  ...
  update: function update(values, returning) {
    ...
    if (!_isEmpty(ret)) this.returning(ret);
  ...
  modify: function modify(callback) {
    callback.apply(this, [this].concat(_tail(arguments)));
    return this;
  },
  ...
  _clearGrouping: function _clearGrouping(grouping) {
    this._statements = _reject(this._statements, { grouping: grouping });
  }
});
