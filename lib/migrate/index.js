...
var CONFIG_DEFAULT = _Object$freeze({
  extension: 'js',
...
var Migrator = function () {
  ...
  Migrator.prototype.latest = function latest(config) {
    ...
    return this._migrationData().tap(validateMigrationList).spread(function (all, completed) {
      var migrations = _difference(all, completed);

      var transactionForAll = !_this.config.disableTransactions && _isEmpty(_filter(migrations, function (name) {
        var migration = require(path.join(_this._absoluteConfigDir(), name));
  ...
  Migrator.prototype.rollback = function rollback(config) {
    ...
    return Promise.try(function () {
      ...
      return _this2._migrationData().tap(validateMigrationList).then(function (val) {
        ...
        return _this2._runBatch(_map(migrations, 'name'), 'down');
      });
    });
  };
  ...
  Migrator.prototype.status = function status(config) {
    ...
    return Promise.all([getTable(this.knex, this.config.tableName, this.config.schemaName).select('*'), this._listAll()]).spread(function (db, code) {
  ...
  Migrator.prototype.currentVersion = function currentVersion(config) {
    ...
    return this._listCompleted().then(function (completed) {
      var val = _max(_map(completed, function (value) {
        return value.split('_')[0];
      }));
      return _isUndefined(val) ? 'none' : val;
    });
  };
  ...
  Migrator.prototype._listAll = function _listAll(config) {
    ...
    return Promise.promisify(fs.readdir, { context: fs })(this._absoluteConfigDir()).then(function (migrations) {
      return _filter(migrations, function (value) {
        var extension = path.extname(value);
        return _includes(loadExtensions, extension);
      }).sort();
    });
  };
  ...
  Migrator.prototype._runBatch = function _runBatch(migrations, direction, trx) {
    ...
    .then(function () {
      ...
      return migrations = _difference(migrations, completed);
    }).then(function () {
      return Promise.all(_map(migrations, _bind(_this7._validateMigrationStructure, _this7)));
    }).then(function () {
  ...
  Migrator.prototype._generateStubTemplate = function _generateStubTemplate() {
    ...
    return Promise.promisify(fs.readFile, { context: fs })(stubPath).then(function (stub) {
      return _template(stub.toString(), { variable: 'd' });
    });
  };
  ...
  Migrator.prototype._useTransaction = function _useTransaction(migration, allTransactionsDisabled) {
    var singleTransactionValue = _get(migration, 'config.transaction');

    return _isBoolean(singleTransactionValue) ? singleTransactionValue : !allTransactionsDisabled;
  };
  ...
  Migrator.prototype._waterfallBatch = function _waterfallBatch(batchNo, migrations, direction, trx) {
    ...
    var log = [];
    _each(migrations, function (migration) {
      var name = migration;
  ...
  Migrator.prototype.setConfig = function setConfig(config) {
    return _assign({}, CONFIG_DEFAULT, this.config || {}, config);
  };
...
function validateMigrationList(migrations) {
  ...
  var diff = _difference(completed, all);
  if (!_isEmpty(diff)) {
